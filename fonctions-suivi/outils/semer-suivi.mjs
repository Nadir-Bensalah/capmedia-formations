...
async function main() {
  ...
  const sebastien = await compte('sebastien.essai@exemple.test', 'Sébastien Horemans', { equipe: false, projets: ['forgeme'] });
  ...
  await bdd.doc('organisations/perseus').set({
    nom: 'Sébastien Horemans', entreprise: 'Perseus Capital', email: 'sebastien.essai@exemple.test', telephone: '', adresse: 'Bruxelles',
    notesInternes: 'Client fondateur de ForgeMe. Point hebdo le mardi.', contacts: [{ nom: 'Sébastien Horemans', email: 'sebastien.essai@exemple.test', role: 'owner', uid: sebastien }],
  ...
  await bdd.doc('projets/forgeme').set({
    ...
    type: 'application-mobile', statut: 'en-cours', organisation: 'perseus',
    client: { nom: 'Sébastien Horemans', email: 'sebastien.essai@exemple.test', entreprise: 'Perseus Capital' },
  ...
  const tickets = [
    { id: 't-veille', numero: 'FORGEME-004', titre: 'Les validations de tâches de la veille ne fonctionnent plus', description: "Je coche une tâche d'hier, elle repasse en non validée aussitôt. Sur aujourd'hui, tout va bien.", etapes: "Ouvrir l'écran Tâches, revenir à la veille, cocher une tâche.", attendu: 'La tâche reste validée.', obtenu: 'Elle revient en non validée dans la seconde.', type: 'bug', urgence: 'bloquant', statut: 'en-cours', auteur: cote(sebastien, 'Sébastien Horemans', 'sebastien.essai@exemple.test'), assigne: nadir, cree: ilYA(0, 6), maj: ilYA(0, 2), resolu: null, lu: { client: ilYA(0, 5), equipe: ilYA(0, 2) } },
    { id: 't-anniv', numero: 'FORGEME-003', titre: 'La liste des anniversaires reste incomplète après un rafraîchissement', description: 'Sur la liste des anniversaires, une partie seulement s\'affiche.', type: 'bug', urgence: 'important', statut: 'a-valider', auteur: cote(sebastien, 'Sébastien Horemans', 'sebastien.essai@exemple.test'), assigne: nadir, cree: ilYA(9), maj: ilYA(1), resolu: null, lu: { client: ilYA(8), equipe: ilYA(1) } },
    { id: 't-export', numero: 'FORGEME-002', titre: 'Pouvoir exporter mes tâches en tableur', description: 'Ce serait pratique pour ma comptabilité.', type: 'fonctionnalite', urgence: 'mineur', statut: 'acceptee', qualification: 'a-chiffrer', auteur: cote(sebastien, 'Sébastien Horemans', 'sebastien.essai@exemple.test'), assigne: nadir, cree: ilYA(20), maj: ilYA(2), resolu: null, lu: { client: ilYA(2), equipe: ilYA(2) } },
    { id: 't-tickets', numero: 'FORGEME-001', titre: 'Une tâche visible sur le web et absente de l\'application', description: 'La tâche « Tickets restau » apparaît sur le web mais pas sur mon téléphone.', type: 'bug', urgence: 'critique', statut: 'resolu', auteur: cote(sebastien, 'Sébastien Horemans', 'sebastien.essai@exemple.test'), assigne: nadir, cree: ilYA(14), maj: ilYA(2), resolu: ilYA(2), lu: { client: ilYA(2), equipe: ilYA(2) } },
    { id: 't-nouveau', numero: null, titre: 'Les notifications arrivent deux fois le matin', description: 'Depuis deux jours, je reçois chaque rappel en double vers huit heures.', type: 'bug', urgence: 'important', statut: 'nouveau', auteur: cote(sebastien, 'Sébastien Horemans', 'sebastien.essai@exemple.test'), assigne: null, cree: ilYA(0, 1), maj: ilYA(0, 1), resolu: null, lu: { client: ilYA(0, 1), equipe: null } },
  ];
  ...
  const messages = [
    ...
    ['t-veille', { uid: sebastien, nom: 'Sébastien Horemans', cote: 'client' }, 'Merci, je guette la mise à jour.', false, ilYA(0, 2)],
  ...
  const evenements = [
    ['t-veille', 'creation', null, 'nouveau', cote(sebastien, 'Sébastien Horemans', 'sebastien.essai@exemple.test'), ilYA(0, 6)],
    ...
    ['t-anniv', 'creation', null, 'nouveau', cote(sebastien, 'Sébastien Horemans', 'sebastien.essai@exemple.test'), ilYA(9)],
  ...
  await bdd.collection('projets/forgeme/messages').add({ de: { uid: nadir, nom: 'Nadir Ben Salah', cote: 'equipe' }, texte: 'Bonjour Sébastien, la version 1.2 arrive sur TestFlight cette semaine.', pieces: [], date: ilYA(1, 2) });
  await bdd.collection('projets/forgeme/messages').add({ de: { uid: sebastien, nom: 'Sébastien Horemans', cote: 'client' }, texte: 'Parfait, je teste dès réception.', pieces: [], date: ilYA(1) });
  ...
  const fichiers = [
    ...
    { categorie: 'captures', nom: 'capture-anniversaires.png', chemin: 'projets/forgeme/documents/client/capture.png', taille: 350000, type: 'image/png', par: { uid: sebastien, nom: 'Sébastien Horemans', cote: 'client' }, visibilite: 'client', version: '' },
  ...
  /* --- Les réunions ------------------------------------------------------------ */
  await bdd.collection('reunions').add({ projet: 'forgeme', titre: 'Point hebdomadaire', date: dans(2, 3), duree: 45, participants: [{ nom: 'Sébastien Horemans' }, { nom: 'Nadir Ben Salah' }], lien: 'https://meet.google.com/abc-defg-hij', ordreDuJour: '1. Retours Android\n2. Planning TestFlight\n3. Questions', notes: '', compteRendu: '', decisions: '', actions: [], visibilite: 'client', par: parNadir, cree: ilYA(5), maj: ilYA(5) });
  await bdd.collection('reunions').add({ projet: 'forgeme', titre: 'Revue de la version 1.1', date: ilYA(7), duree: 60, participants: [{ nom: 'Sébastien Horemans' }, { nom: 'Nadir Ben Salah' }], lien: '', ordreDuJour: 'Revue des écrans livrés.', notes: '', compteRendu: 'Écrans validés. Deux retours sur les couleurs, pris en compte.', decisions: 'Conserver Stripe pour les paiements.', actions: [{ texte: 'Envoyer les captures du store', fait: false }], visibilite: 'client', par: parNadir, cree: ilYA(10), maj: ilYA(7) });

  /* --- Les notes et décisions --------------------------------------------------- */
  await bdd.collection('notes').add({ projet: 'forgeme', type: 'decision', titre: 'Conserver Stripe pour les paiements', contenu: 'Après comparaison avec RevenueCat seul, Stripe reste pour le web.', contexte: 'Revue de la version 1.1.', impact: 'Aucun changement de code côté mobile.', decidePar: 'Sébastien et Nadir', date: ilYA(7), visibilite: 'client', par: parNadir, cree: ilYA(7), maj: ilYA(7) });
  ...
  const activite = [
    ...
    { projet: 'forgeme', type: 'demande', texte: 'a ouvert la demande « Les notifications arrivent deux fois le matin »', par: { uid: sebastien, nom: 'Sébastien Horemans', cote: 'client' }, lien: '/projets/forgeme/demandes/t-nouveau', visibilite: 'client', date: ilYA(0, 1) },
  ...
  await bdd.doc(`profils/${sebastien}`).set({ nom: 'Sébastien Horemans', email: 'sebastien.essai@exemple.test', derniereVisite: ilYA(2), notifications: {}, lus: {} });
