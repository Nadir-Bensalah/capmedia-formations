...
await env.withSecurityRulesDisabled(async (ctx) => {
  ...
  await setDoc(doc(b, 'organisations/[nom retire]'), { nom: 'Seb', membres: [SEB], contacts: [] });
  await setDoc(doc(b, 'organisations/menuo'), { nom: 'Claire', membres: [CLAIRE], contacts: [] });
  await setDoc(doc(b, 'projets/forgeme'), { nom: 'ForgeMe', ref: 'FORGEME', membres: [SEB], organisation: '[nom retire]', statut: 'en-cours', compteur: 0 });
...
await doit('[nom retire] lit son projet', getDoc(doc(seb(), 'projets/forgeme')));
await refuse('[nom retire] ne lit pas le projet de Claire', getDoc(doc(seb(), 'projets/menuo')));
await refuse('[nom retire] ne liste pas tous les projets', getDocs(collection(seb(), 'projets')));
await doit('[nom retire] liste les projets où il est membre', getDocs(query(collection(seb(), 'projets'), where('membres', 'array-contains', SEB))));
await refuse('Claire ne lit pas la demande de [nom retire]', getDoc(doc(claire(), 'tickets/t1')));
await refuse('Claire ne lit pas la validation de [nom retire]', getDoc(doc(claire(), 'validations/v1')));
...
await refuse("Claire ne lit pas l'organisation de [nom retire]", getDoc(doc(claire(), 'organisations/[nom retire]')));
await refuse('Anonyme ne lit rien', getDoc(doc(anonyme(), 'projets/forgeme')));
await refuse('[nom retire] ne lit pas la boîte de Claire', getDocs(collection(seb(), `boites/${CLAIRE}/notifications`)));

console.log('\n== Interne contre client');
await refuse('[nom retire] ne lit pas une tâche interne', getDoc(doc(seb(), 'taches/t-interne')));
await doit('[nom retire] lit une tâche visible', getDoc(doc(seb(), 'taches/t-client')));
await refuse('[nom retire] ne liste pas les tâches sans le filtre de visibilité', getDocs(query(collection(seb(), 'taches'), where('projet', '==', 'forgeme'))));
await doit('[nom retire] liste les tâches visibles', getDocs(query(collection(seb(), 'taches'), where('projet', '==', 'forgeme'), where('visibilite', '==', 'client'))));
await refuse('[nom retire] ne lit pas une note interne', getDoc(doc(seb(), 'notes/n-interne')));
await refuse('[nom retire] ne lit pas un fichier interne', getDoc(doc(seb(), 'fichiers/f-interne')));
await refuse('[nom retire] ne lit pas un lien interne', getDoc(doc(seb(), 'projets/forgeme/liens/prive')));
await doit('[nom retire] lit un lien public', getDoc(doc(seb(), 'projets/forgeme/liens/public')));
await refuse("[nom retire] ne lit pas l'activité interne", getDoc(doc(seb(), 'activite/a-interne')));
await doit("[nom retire] lit l'activité client", getDoc(doc(seb(), 'activite/a-client')));
await refuse('[nom retire] ne lit pas une note interne de demande', getDoc(doc(seb(), 'tickets/t1/messages/m-interne')));
await doit('[nom retire] lit un message public de demande', getDoc(doc(seb(), 'tickets/t1/messages/m-public')));
await doit("L'équipe lit tout, interne compris", getDoc(doc(equipe(), 'taches/t-interne')));

console.log('\n== Ce que le client peut écrire');
await doit('[nom retire] crée une demande', addDoc(collection(seb(), 'tickets'), { numero: null, projet: 'forgeme', composant: '', titre: 'Bug', description: 'x', type: 'bug', urgence: 'important', statut: 'nouveau', plateforme: 'ios', version: '', etapes: '', attendu: '', obtenu: '', contexte: '', appareil: '', liens: [], assigne: null, auteur: { uid: SEB, nom: 'Seb', email: 'sebastien.essai@exemple.test', cote: 'client' }, pieces: [], archive: false, cree: serverTimestamp(), maj: serverTimestamp(), resolu: null, lu: { client: null, equipe: null }, qualification: null, devis: null }));
await refuse('[nom retire] ne crée pas une demande sur le projet de Claire', addDoc(collection(seb(), 'tickets'), { numero: null, projet: 'menuo', composant: '', titre: 'Bug', description: 'x', type: 'bug', urgence: 'important', statut: 'nouveau', plateforme: 'ios', version: '', etapes: '', attendu: '', obtenu: '', contexte: '', appareil: '', liens: [], assigne: null, auteur: { uid: SEB, nom: 'Seb', email: 'sebastien.essai@exemple.test', cote: 'client' }, pieces: [], archive: false, cree: serverTimestamp(), maj: serverTimestamp(), resolu: null, lu: { client: null, equipe: null }, qualification: null, devis: null }));
await refuse('[nom retire] ne se donne pas un numéro', addDoc(collection(seb(), 'tickets'), { numero: 'FORGEME-999', projet: 'forgeme', composant: '', titre: 'Bug', description: 'x', type: 'bug', urgence: 'important', statut: 'nouveau', plateforme: 'ios', version: '', etapes: '', attendu: '', obtenu: '', contexte: '', appareil: '', liens: [], assigne: null, auteur: { uid: SEB, nom: 'Seb', email: 'sebastien.essai@exemple.test', cote: 'client' }, pieces: [], archive: false, cree: serverTimestamp(), maj: serverTimestamp(), resolu: null, lu: { client: null, equipe: null }, qualification: null, devis: null }));
await doit('[nom retire] valide une correction livrée', updateDoc(doc(seb(), 'tickets/t1'), { statut: 'resolu', resolu: serverTimestamp(), maj: serverTimestamp(), 'lu.client': serverTimestamp() }));
await refuse("[nom retire] ne change pas l'urgence", updateDoc(doc(seb(), 'tickets/t1'), { urgence: 'bloquant' }));
await refuse("[nom retire] ne s'assigne pas la demande", updateDoc(doc(seb(), 'tickets/t1'), { assigne: SEB }));
await doit('[nom retire] écrit dans la conversation', addDoc(collection(seb(), 'projets/forgeme/messages'), { de: { uid: SEB, nom: 'Seb', cote: 'client' }, texte: 'Bonjour', pieces: [], date: serverTimestamp() }));
await refuse("[nom retire] ne se fait pas passer pour l'équipe", addDoc(collection(seb(), 'projets/forgeme/messages'), { de: { uid: SEB, nom: 'Seb', cote: 'equipe' }, texte: 'Bonjour', pieces: [], date: serverTimestamp() }));
await refuse("[nom retire] n'écrit pas une note interne", addDoc(collection(seb(), 'tickets/t1/messages'), { de: { uid: SEB, nom: 'Seb', cote: 'client' }, texte: 'x', pieces: [], interne: true, date: serverTimestamp() }));
await doit('[nom retire] approuve une validation', updateDoc(doc(seb(), 'validations/v1'), { statut: 'approuvee', reponse: { par: SEB, nom: 'Seb', date: serverTimestamp(), commentaire: 'ok' }, maj: serverTimestamp() }));
await refuse('[nom retire] ne répond pas deux fois', updateDoc(doc(seb(), 'validations/v1'), { statut: 'modifications', reponse: { par: SEB, nom: 'Seb', date: serverTimestamp(), commentaire: 'non' }, maj: serverTimestamp() }));
await refuse('[nom retire] ne crée pas une validation', addDoc(collection(seb(), 'validations'), { projet: 'forgeme', titre: 'x', statut: 'en-attente' }));
await doit('[nom retire] dépose une capture', addDoc(collection(seb(), 'fichiers'), { projet: 'forgeme', composant: '', categorie: 'captures', nom: 'c.png', chemin: 'projets/forgeme/documents/client/c.png', taille: 1, type: 'image/png', description: '', tags: [], par: { uid: SEB, nom: 'Seb', cote: 'client' }, visibilite: 'client', version: '', archive: false, cree: serverTimestamp() }));
await refuse('[nom retire] ne dépose pas dans « contrats »', addDoc(collection(seb(), 'fichiers'), { projet: 'forgeme', composant: '', categorie: 'contrats', nom: 'c.pdf', chemin: 'projets/forgeme/documents/client/c.pdf', taille: 1, type: 'application/pdf', description: '', tags: [], par: { uid: SEB, nom: 'Seb', cote: 'client' }, visibilite: 'client', version: '', archive: false, cree: serverTimestamp() }));
await refuse("[nom retire] ne dépose pas sous le chemin d'un autre projet", addDoc(collection(seb(), 'fichiers'), { projet: 'forgeme', composant: '', categorie: 'captures', nom: 'c.png', chemin: 'projets/menuo/documents/client/c.png', taille: 1, type: 'image/png', description: '', tags: [], par: { uid: SEB, nom: 'Seb', cote: 'client' }, visibilite: 'client', version: '', archive: false, cree: serverTimestamp() }));
await doit('[nom retire] accepte un devis', updateDoc(doc(seb(), 'documents/d1'), { statut: 'accepte', reponse: { par: SEB, nom: 'Seb', date: serverTimestamp(), commentaire: '' } }));
await refuse('[nom retire] ne touche pas au montant', updateDoc(doc(seb(), 'documents/d1'), { montant: 1 }));
await refuse('[nom retire] ne marque pas une facture payée', updateDoc(doc(seb(), 'documents/f1'), { statut: 'payee' }));
await doit('[nom retire] marque une notification lue', updateDoc(doc(seb(), `boites/${SEB}/notifications/n1`), { lu: true }));
await refuse('[nom retire] ne se crée pas une notification', addDoc(collection(seb(), `boites/${SEB}/notifications`), { titre: 'x', lu: false }));
await doit('[nom retire] écrit son profil', setDoc(doc(seb(), `profils/${SEB}`), { nom: 'Seb', notifications: { messages: 'off' } }, { merge: true }));
await refuse("[nom retire] n'écrit pas le profil de Claire", setDoc(doc(seb(), `profils/${CLAIRE}`), { nom: 'x' }));
...
await refuse("[nom retire] ne lit pas la demande de projet de Claire", getDoc(doc(seb(), 'demandesProjet/dp1')));

console.log('\n== Ce que personne ne fait depuis le navigateur');
await refuse("[nom retire] n'écrit pas l'activité", addDoc(collection(seb(), 'activite'), { projet: 'forgeme', type: 'x', texte: 'x', visibilite: 'client' }));
...
await refuse('[nom retire] ne lit pas les prospects du site', getDoc(doc(seb(), 'contact-messages/c1')));
