...
export const nouveau = async (ctx, env) => {
  ...
  const d = {
    ...
    liens: [], inviter: false, demandeProjet: depuis ? depuis.id : '',
  };
  let etape = 0;
  ...
  const corps = () => {
    switch (etape) {
      ...
      case 8: return `${encart("<strong>Le projet se crée rideau baissé.</strong> Personne n'y a accès tant que vous ne l'ouvrez pas : vous le garnissez, vous le chiffrez, et vous levez le rideau quand il est présentable. Le bouton « Ouvrir au client » vous attend sur la page du projet.", 'info', 'oeilFerme')}
        <label class="interrupteur" style="margin-top:14px"><input type="checkbox" name="inviter" ${d.inviter ? 'checked' : ''}><i></i> Ouvrir tout de suite, sans rien préparer</label>
        <p class="aide" style="margin-top:8px">À ne cocher que si l'espace est déjà prêt ailleurs. Le contact reçoit son invitation dans la foulée.</p>`;
      case 9: { const o = orgChoisie(); return `<dl class="faits" style="grid-template-columns:1fr 1fr">${[['Projet', `${d.nom} (${d.ref.toUpperCase()})`], ['Client', o ? (o.entreprise || o.nom) : `${d.clientEntreprise} · ${d.clientNom} · ${d.clientEmail}`], ['Type', TYPES_PROJET[d.type]], ['Statut', (STATUTS_PROJET[d.statut] || {}).libelle], ['Les parties', d.composants.map((c) => c.nom).join(', ') || 'Aucune'], ['Responsable', (equipe.find((e) => e.id === d.responsable) || {}).nom || ''], ['Dates', [d.debut, d.cible].filter(Boolean).join(' → ') || 'Non fixées'], ['Budget', d.budget ? `${d.budget} € HT` : 'Non renseigné'], ['Liens', d.liens.length ? pluriel(d.liens.length, 'lien') : 'Aucun'], ['Ouverture au client', d.inviter ? 'Tout de suite' : 'Plus tard, rideau baissé']].map(([l, v]) => `<div class="fait"><dt>${echapper(l)}</dt><dd>${echapper(v)}</dd></div>`).join('')}</dl>${depuis ? encart(`Ce projet reprend la demande « ${echapper(depuis.titre)} ». Sa discussion et ses fichiers restent accessibles depuis la fiche de la demande.`, 'info', 'sparkle') : ''}`; }
      default: return '';
    }
  };
