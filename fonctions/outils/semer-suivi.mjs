...
const semer = async () => {
  ...
  const sebastien = await compte('sebastien@horemans.eu', 'Sébastien Horemans', { projets: ['forgeme'] });
  ...
  await bdd.doc('projets/forgeme').set({
    ...
    client: { nom: 'Sébastien Horemans', email: 'sebastien@horemans.eu', entreprise: 'Perseus Capital' },
    membres: [sebastien],
  ...
  const cote = (uid) => uid === nadir
    ...
    : { uid: sebastien, nom: 'Sébastien Horemans', email: 'sebastien@horemans.eu', cote: 'client' };
  ...
  const messages = [
    ...
    ['t-veille', { uid: nadir, nom: 'Nadir Ben Salah', cote: 'equipe' },
     ...
     true, ilYA(0, 3)],
    ['t-veille', { uid: sebastien, nom: 'Sébastien Horemans', cote: 'client' },
  ...
  console.log('  clients  sebastien@horemans.eu (ForgeMe), claire@exemple.fr (Menuo)');
